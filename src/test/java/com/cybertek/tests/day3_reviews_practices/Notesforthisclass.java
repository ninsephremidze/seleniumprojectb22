...
NoSuchElementException
When do you get NoSuchElementException? #1- Your locator is not correct. #2- WebElement might not be loaded into the page yet. In this case we need to adjust the speed of the BrowserDriver's speed to the browser's speed.
...
syntax1: driver.findElement(By.locator("something")).sendKeys("string" + Keys.ENTER); syntax2: driver.findElement(By.locator("something")).sendKeys("string" , Keys.ENTER);
...
#1- id : - It looks through the id attribute values and returns the matching web element. - id is a unique attribute value. - it must be unique for each web element. - sometimes it might have "dynamic" id attribute value.

#2- linkText : - linkText locator will find and return the LINK by its text. - the text that is in between the opening tag and closing tag is used to locate web element. - IT ONLY WORKS WITH LINKS. - It will LOOK FOR EXACT TEXT MATCH. - We can compare this method to .equals() from Java

#3- partialLinkText : - partialLinkText locator is VERY SIMILAR TO LINKTEXT - This locator will only look for partial text match. - We can compare this method to .contains() from Java

#4- name : - this locator locates using name attribute's value - Does this have to be unique value? - NO.

#5- className : - this locator locates using CLASS attribute's value to locate web element - Does this have to be unique value? - NO.

#6- tagName - this locator uses TAGNAME to locate web elements - this is usually not very useful for locating SINGLE web elements - this locator is useful when you are trying to locate multiple web elements
